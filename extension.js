...
class MemoryWarningDialog extends ModalDialog.ModalDialog {
     ...
     */
    constructor({ ramPercent, swapPercent, ramThreshold, swapThreshold, onClose }) {
        super({
        ...
        if (ramPercent >= ramThreshold) {
            lines.push(`RAM usage: ${ramPercent.toFixed(1)}%  (threshold: ${ramThreshold}%)`);
        }
        if (swapPercent >= swapThreshold) {
            lines.push(`Swap usage: ${swapPercent.toFixed(1)}%  (threshold: ${swapThreshold}%)`);
        }

        const body = new St.Label({
            text: 'Memory consumption has exceeded the configured threshold.\n'
...
export default class MemoryGuardExtension extends Extension {
    ...
    _checkMemory() {
        ...
        const ramPercent = info.memTotal > 0
            ? (ramUsed / info.memTotal) * 100
            : 0;

        // Swap used = Total − Free
        const swapUsed = info.swapTotal - info.swapFree;
        const swapPercent = info.swapTotal > 0
            ? (swapUsed / info.swapTotal) * 100
            : 0;
        ...
        if (ramExceeded || swapExceeded) {
            this._showWarningDialog(ramPercent, swapPercent, ramThreshold, swapThreshold);
        }
    }
